  ...
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-primary/20 selection:text-primary">
      ...
      <section className="py-12 px-6 max-w-5xl mx-auto space-y-8">
        <GlassPanel className="p-6 md:p-8 space-y-6 border-primary/30">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <h2 className="text-xl font-bold text-foreground">Interactive DCA Guardrail Simulator</h2>
              <p className="text-xs text-muted-foreground mt-0.5">
                Drag to see how MITIGATOR automatically splits large orders into staged tranches to safeguard execution price.
              </p>
            </div>
            <span className="text-xs font-mono font-bold px-3 py-1 rounded-lg bg-primary/10 text-primary border border-primary/25">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
            <div className="p-4 rounded-xl border border-rose-500/30 bg-rose-500/5 space-y-2">
              ...
              <p className="text-2xl font-bold font-mono text-foreground">${orderSize.toLocaleString()}</p>
            ...
            <div className="p-4 rounded-xl border border-emerald-500/30 bg-emerald-500/10 space-y-2">
              ...
              <div className="flex items-baseline gap-2">
                <span className="text-2xl font-bold font-mono text-emerald-400">${immediateFill.toLocaleString()}</span>
                <span className="text-xs font-mono text-muted-foreground">Now (37.5%)</span>
                <span className="text-2xl font-bold font-mono text-cyan-400 ml-2">${stagedFill.toLocaleString()}</span>
      ...
      <section className="py-12 px-6 max-w-6xl mx-auto space-y-6">
        <div className="text-center space-y-2">
          <h2 className="text-2xl md:text-3xl font-black tracking-tight">Execution Engine Architecture</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {EXECUTION_PILLARS.map((pillar) => (
            <GlassPanel key={pillar.title} className="p-6 space-y-4 hover:border-emerald-500/30 transition-all">
      ...
      <section className="py-20 px-6">
        <div className="max-w-4xl mx-auto rounded-3xl border border-emerald-500/30 bg-card/60 p-8 md:p-12 text-center space-y-6 shadow-2xl">
          ...
          <div className="flex justify-center pt-2">
            <Link
              href="/discover"
              className="flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-500 px-6 py-3.5 text-sm font-bold text-white shadow-lg shadow-emerald-500/25 hover:scale-[1.02] active:scale-[0.98] transition-all cursor-pointer"
            >
