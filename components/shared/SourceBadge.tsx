...
const tierStyles: Record<SourceTier, { bg: string; text: string; label: string }> = {
...
export function SourceBadge({ tier, className }: { tier: SourceTier; className?: string }) {
...
export function FreshnessBadge({ freshness }: { freshness: 'live' | 'recent' | 'stale' | 'delayed' }) {
  const styles = {
...
export function RiskBadge({ level }: { level: string }) {
  ...
  return (
    <span className={cn('inline-flex items-center rounded border px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide', styles[level] || styles.elevated)}>
      {level}
    </span>
  );
}
